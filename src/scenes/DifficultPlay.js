class DifficultPlay extends Phaser.Scene {
    constructor() {
        super("difficultplayScene")
    }
    ...
    create() {
      ...
      //var target = new Phaser.Math.Vector2();
        ...
        // add fast spaceship
        this.fastShip = new Spaceship(this, game.config.width + borderUISize * 6, borderUISize * 4, 'special-spaceship', 0, 100).setOrigin(0, 0);
        this.fastShip.moveSpeed *= 1.5;
        ...
        this.clock = this.time.delayedCall(45000, () => this.gameOvert(), null, this)
    ...
    gameOvert(){
      ...
      let gameOverText = this.add.text(game.config.width/2, game.config.height/2, 'GAME OVER', textConfig).setOrigin(0.5)
      let restartText = this.add.text(game.config.width/2, game.config.height/2 + 64, 'Press (R) to Restart or <- for Menu', textConfig).setOrigin(0.5)

      // 手动设置背景颜色
      gameOverText.setBackgroundColor('#F3B141');
      restartText.setBackgroundColor('#F3B141');
      ...
      this.clock.elapsed = currentTime
        ...
        this.starfield.tilePositionX -= 3//背景移动
        ...
        this.timeRight.text =this.second - Math.floor(this.clock.getElapsedSeconds()) //this.clock.elapsed //
      }
